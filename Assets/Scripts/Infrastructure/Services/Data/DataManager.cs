...
namespace MonsterFactory.Services.DataManagement
{
    ...
    public class DataManager : MFService, IDataManager
    {
        ...
        public MFData GetIfDataExists(Type type)
        {
            if (locallyStoredDataCache == null)
            {
                return null;
            }

            if (runtimeStoredDataCache.ContainsKey(type))
            {
                return runtimeStoredDataCache[type];
            }

            if (locallyStoredDataCache.ContainsKey(type))
            {
                return locallyStoredDataCache[type];
            }
